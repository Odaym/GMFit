...
public class Setup_Profile_3_Fragment extends Fragment {
    ...
    @Bind(R.id.activityLevelsRadioButtonsGroup)
    RadioGroupPlus activityLevelsRadioButtonsGroup;
    ...
    private void getActivityLevels() {
        ...
        DataAccessHandler.getInstance().getActivityLevels(prefs.getString(Constants.PREF_USER_ACCESS_TOKEN, Constants.NO_ACCESS_TOKEN_FOUND_IN_PREFS),
                        ...
                        switch (response.code()) {
                            case 200:
                                ...
                                for (int i = 0; i < activityLevels.size(); i++) {
                                    View listItemRadioButton = getActivity().getLayoutInflater().inflate(R.layout.activity_levels_list_item_radio_button_row, null);

                                    final RadioButton radioButtonItem = (RadioButton) listItemRadioButton.findViewById(R.id.activityLevelRadioButtonItem);
                                    radioButtonItem.setText(activityLevels.get(i).getName());
                                    radioButtonItem.setId(activityLevels.get(i).getId());

                                    TextView radioButtonHintTV = (TextView) listItemRadioButton.findViewById(R.id.radioButtonHintTV);
                                    ...
                                    radioButtonItem.setOnClickListener(new View.OnClickListener() {
                                        ...
                                        public void onClick(View view) {
                                            prefs.edit().putInt(Constants.EXTRAS_USER_PROFILE_ACTIVITY_LEVEL_ID, radioButtonItem.getId()).apply();
                                            prefs.edit().putString(Constants.EXTRAS_USER_PROFILE_ACTIVITY_LEVEL, radioButtonItem.getText().toString()).apply();
                                        }
                                    });
